...
打开浏览器访问：
https://github.com/MetaCubeX/mihomo/releases,下载带 compatible 字样的版本
找到最新版本（Latest），下载以下文件：

普通用户：mihomo-windows-amd64-v版本号.zip
如果是老电脑：mihomo-windows-386-v版本号.zip


解压 zip 文件，得到 mihomo-windows-amd64.exe
新建一个文件夹，例如 C:\mihomo\，把 exe 文件放进去，并重命名为 mihomo.exe
...
powershell   cd C:\mihomo

运行内核：

powershell   .\mihomo.exe -d .
```
...
vbscriptSet ws = CreateObject("WScript.Shell")
...
第三步：如何停止 Mihomo
任务管理器 → 找到 mihomo.exe → 右键 → 结束任务
或者用 PowerShell 一条命令结束：
powershelltaskkill /IM mihomo.exe /F
```
